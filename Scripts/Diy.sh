...
function git_sparse_clone() {
    ...
    target_path=$1
    outname=$(basename "$target_path")
    cd "$repodir" && git sparse-checkout set $@
    mv -f "$target_path" "../$outname"
    cd .. && rm -rf "$repodir"
}
...
echo "🚀 正在下载 AdGuardHome 内核: $ADG_VERSION"
echo "🔗 下载链接: $ADG_URL"
...
# 3. 分别针对不同仓库，写入各自特有的配置内容
if [[ "${REPO_NAME}" == "immortalwrt" ]]; then
    cat >> "${ZZZ}" <<-EOF
# 这里写入 immortalwrt 仓库特有的配置内容
# /etc/config/network
uci add_list network.lan.dns='192.168.9.1' # DNS(多个DNS要用空格分开)
...
uci add_list dhcp.lan.dhcp_option='6,192.168.9.1'
...
uci set dropbear.main.Port='8822' # SSH端口设置为'8822'
